...
export function StoreProvider({ children }) {
  ...
  const [notifications, setNotifications] = useState(() => boot('bsw_notifications', [{ id: 1, type: 'system', text: 'Welcome to BLUESAILWEAR ✨', seen: false }]));
  ...
  const notify = (text, type = 'system') => setNotifications((prev) => [{ id: Date.now() + Math.random(), type, text, seen: false }, ...prev]);
  ...
  const advanceOrderStatus = (orderId) => {
    ...
    setOrders((prev) =>
      prev.map((order) => {
        ...
        changedOrder = { ...order, stage: nextStage, status: trackingStages[nextStage] };
        return changedOrder;
      }),
    );

    if (changedOrder) {
      notify(`Order ${changedOrder.orderNo} is now ${changedOrder.status.replace(/_/g, ' ')}`, 'order');
    }
  };
