...
export default function Contact() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      notify('Unable to reach support API right now. Please try again shortly.', 'system');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={submit} className="glass mx-auto max-w-3xl space-y-3 rounded-2xl p-6">
      ...
      <button disabled={loading} className="rounded bg-cyan-400 px-4 py-2 text-slate-900 disabled:opacity-50">{loading ? 'Sending...' : 'Send Message'}</button>
    </form>
  );
}
